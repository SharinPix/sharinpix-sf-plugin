...
function createOrderedElement(merged: FormElement, elementIndex: number): FormElement {
  const orderedKeys = orderElementKeys(Object.keys(merged).filter((k) => k !== 'index'));
  const element: FormElement = {};

  for (const key of orderedKeys) {
    element[key] = merged[key];
    if (key === 'id') element.index = elementIndex;
  }
  if (!('index' in element)) element.index = elementIndex;
  return element;
}
...
async function processCsvFile(filePath: string): Promise<ProcessResult> {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const merged: FormElement = { ...baseWithoutCleared, ...Object.fromEntries(values) };
      elements.push(createOrderedElement(merged, elementIndex));
    }
