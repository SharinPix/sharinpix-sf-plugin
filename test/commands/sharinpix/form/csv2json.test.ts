...
describe('sharinpix form csv2json', () => {
  ...
  it('should round-trip CSV -> JSON -> CSV with mixed datatypes', async () => {
    const csvContent = [
      'id,index,required,rows,options,sfMapping,label',
      'elem1,1e3,false,0,[],"[{""a"":1}]","Question, line1\nline2"',
    ].join('\n');
    ...
    expect(record.index).to.equal('1000');
    expect(record.required).to.equal('false');
    expect(record.rows).to.equal('0');
  ...
  it('should skip CSV conversion when the CSV includes unknown keys', async () => {
    const csvContent = ['id,index,required,rows,customKey', 'elem1,0,true,3,{"a":1}'].join('\n');
